...
const FORMATS: Array<{
...
}> = [
  {
    ...
    desc: "Native Microsoft Excel workbook with formatted columns.",
    tone: "emerald",
    pro: false,
  },
  {
    ...
    ext: ".csv",
    icon: FileText,
    desc: "Universal comma-separated. Opens in anything.",
    tone: "slate",
    pro: false,
  },
  {
    ...
    ext: ".xml",
    icon: FileCode,
    desc: "Direct import into Tally Prime / ERP 9 daybook.",
    tone: "slate",
    pro: true,
  },
  {
    ...
    ext: ".ofx",
    icon: FileType,
    desc: "Open Financial Exchange — Quicken, Money.",
    tone: "slate",
    pro: true,
  },
  {
    ...
    ext: ".qif",
    icon: FileType,
    desc: "Legacy Quicken import format.",
    tone: "slate",
    pro: true,
  },
  {
    ...
    ext: ".qbo",
    icon: FileType,
    desc: "QuickBooks Web Connect — imports as a bank feed.",
    tone: "slate",
    pro: true,
  },
  {
    ...
    ext: ".iif",
    icon: FileType,
    desc: "QuickBooks Desktop import format.",
    tone: "slate",
    pro: true,
  },
];
...
function ExportPage() {
  ...
  const currency = useMemo(
    () => statements.find((st) => st.currency)?.currency ?? null,
    [statements],
  );
  ...
  const baseFileName = useMemo(() => {
    ...
    return statements.length > 1
      ? `${first}-and-${statements.length - 1}-more`
      : first || "statement";
  }, [statements]);

  const includedRows = useMemo(
    () =>
      options.omitLowConfidence
        ? rows.filter((r) => getConfidenceTier(r.confidence) !== "low")
        : rows,
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Choose a format, configure options, then download.
          </p>
        </div>
            ...
            <div className="lg:sticky lg:top-20 space-y-3">
              <div className="rounded-xl border border-border bg-ink p-4 text-background">
                ...
                <dl className="mt-3 space-y-2 text-xs">
                  <SummaryRow
                    label="Format"
                    value={`${FORMATS.find((f) => f.key === selected)?.name} (${FORMATS.find((f) => f.key === selected)?.ext})`}
                  />
                ...
                >
                  ...
                  {downloaded[selected]
                    ? `Re-download ${FORMATS.find((f) => f.key === selected)?.ext}`
                    : `Download ${FORMATS.find((f) => f.key === selected)?.ext}`}
                </button>
                {FORMATS.find((f) => f.key === selected)?.pro && (
                  <p className="mt-2 text-center text-[11px] text-amber-400/90">
                    {FORMATS.find((f) => f.key === selected)?.name} requires Lifetime
                  </p>
                )}
              </div>
              ...
              <div className="flex items-start gap-2 rounded-lg border border-emerald/30 bg-emerald-soft/40 px-3 py-2.5 text-xs text-accent-foreground">
                ...
                <span>
                  File is generated on this device and downloaded directly. No data is transmitted.
                </span>
              </div>
            </div>
          </div>
