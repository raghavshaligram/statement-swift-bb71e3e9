...
const RULES: Rule[] = [
  // --- Bank machinery: highest specificity, must beat merchant rules ---
  {
    ...
    patterns: [
      ...
      // IMPS transfer.
      /\bCHRG\s*\/\s*(IMPS|NEFT|RTGS|UPI|ATM)\b/i,
      /^CHRG\b/i,
    ],
    confidence: 92,
  },
 ...
 */
const P2P_RAILS =
  /\b(UPI|IMPS|NEFT|ZELLE|INTERAC|VENMO|CASH APP|PAYTM|PHONEPE|GPAY|E-?TRANSFER)\b/i;
...
const BUSINESS_SUFFIX =
  /\b(LTD|LIMITED|LLC|LLP|INC|PVT|PLC|GMBH|CORP|CO|COMPANY|SERVICES|SOLUTIONS|ENTERPRISES|TRADING|TECHNOLOGIES|STORES?)\b/i;
 ...
 */
export function measureCoverage(transactions: { description: string; amount: number }[]): {
  total: number;
  matched: number;
  coverage: number;
  unmatched: string[];
